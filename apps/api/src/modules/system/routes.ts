...
import { activeAlerts, alertHistory } from './alerts.ts'
import { isRunning } from './diagnostics.ts'
import { overall, probeAll } from '../../infrastructure/observability/probes.ts'
import { enqueue } from '../../infrastructure/queue/index.ts'
import { compare, thresholds, worst } from './thresholds.ts'

import type { Level, MetricKey, Threshold } from './thresholds.ts'
...
export const adminMonitoring: FastifyPluginAsync = async fastify => {
  fastify.addHook('onRequest', fastify.requirePermission('system.metrics.view', { hide: true }))
  ...
  fastify.post('/diagnostics', {
    onRequest: fastify.requirePermission('system.diagnostics.run', { hide: true })
  }, async (request, reply) => {
