...
the only `<head>` anything outside a browser could read was `apps/web/index.html`'s
...
* **`index.html` references its assets from the root** (`/js/app.js`, not
  ...
  type. The page loaded with no scripts and no styles at all. `apps/web/test/
...
| `apps/api/test/seo.test.ts` | escaping, what is refused, the origin |
| `apps/web/test/path-routes.test.mjs` | the router's half of the agreement |
