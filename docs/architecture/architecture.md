...
Schema in `database/migrations/`, one file per domain, fully commented. See
[database.md](../database/database.md). Scaling path: read replicas for catalogue
...
title always outranks a fuzzy near-miss. See [`../database/search.md`](../database/search.md).
...
Workers are separate deployables in `apps/api/src/workers/`
...
Nothing is stored outside the database today; see [redis.md](../operations/redis.md) for the
...
`apps/web/js/catalogue.js` is the only module that knows both vocabularies. It maps
...
`apps/web/js/catalogue.js` implements this rule and `apps/web/test/catalogue.test.mjs`
pins it.
...
All of them, as of the browse rewiring. `apps/web/js/catalogue.js` presents
