...
> exercised by the integration tests. The route list the server actually
> registers is in `apps/api/src/app.ts`; when this document and the code
...
| GET | `/v1/anime/search` | tiered search: `?q` plus `&genre&year&season&format&status&sort&limit&offset&nsfw`. Matches canonical, romaji/english/native and synonym titles; each row reports its `tier` and `matched_title`. See [`../database/search.md`](../database/search.md) |
