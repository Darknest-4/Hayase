...
`apps/api/test/search-performance.test.ts` asserts the mechanism — which query
...
`apps/api/test/metadata.test.ts` (16) — every precedence branch, lock
behaviour, empty/unchanged handling, volatile fields, title normalisation.
`apps/api/test/search.test.ts` (16) — tier definitions, filter composition,
