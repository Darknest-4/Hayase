...
> Frissítve: 2026‑08‑25, és azóta elavult: a fájlstruktúrát átszerveztük
> (`server/` → `apps/api`, `web/` → `apps/web`, `db/` → `database/`). A
> `status.html` ugyanennek a napnak az archív pillanatképe, nem aktuális
> leltár. Az élő leírás: `docs/architecture/architecture.md`.

A magas szintű, elrendezés‑központú áttekintés a [`../architecture/attekintes.md`](../architecture/attekintes.md)‑ben van;
...
| Réteg | Technológia |
|-------|-------------|
| **Web kliens** | Keretrendszer nélküli HTML/CSS/JS SPA (`web/`), saját hash‑router (`app.js`), `U.el()` DOM‑helper, design tokenek (`apps/web/css/tokens.css`) — fekete háttér, rózsa akcent, Nunito, keskeny ikon‑sidebar |
...
- [x] **Capability‑aware service‑próbák**: Postgres kemény függőség; Redis/RabbitMQ/OpenSearch/MinIO **csak ha konfigurálva van** → nincs hamis riasztás (a próbák megmaradtak; a compose‑ból azóta kikerült minden, aminek nincs feladata — indoklás: `docs/operations/redis.md`)
...
### Security hardening (kész) — [`../security/security.md`](../security/security.md)
...
### Streaming 2.0 — forrás‑absztrakció (kész) — [`../integrations/streaming.md`](../integrations/streaming.md)
...
- [x] **Nincs OpenSearch — szándékosan**: 25 672 sornál a Postgres `pg_trgm`+`tsvector` a 0017 indexekről ezredmásodpercben válaszol; az OpenSearch ~1 GB RAM‑ot és egy külön üzemeltetett JVM‑et kérne mérhető haszon nélkül (indoklás és a felülvizsgálat feltétele: `docs/database/search.md`)
...
- [x] **Tartalom‑címzett tároló** (`apps/api/src/lib/package-store.ts`): a kulcs **maga a bájtok sha256‑ja**, amit a szerver számol — a publikáló nem állíthatja meg, mire hashelnek a saját bájtjai; azonos tartalom egy blobra esik össze; atomi írás (temp + rename), tehát csonka blob sosem kap érvényesnek látszó nevet
...
- [x] Kliens‑modul: `apps/web/js/library-sync.js` (`LibrarySync`)
...
Migrációk: `0001`…`0017` (a `database/migrations/`‑ben, filename szerint követve a `schema_migrations`‑ben).
...
## 6. Frontend oldalak (`apps/web/js/pages/`)
...
A 2026‑08‑25‑i állapot archív pillanatképe: `docs/operations/status.html` (elavult).
...
### Hasznos scriptek (`apps/api/package.json`)
